...
def make_variant(skip_ridge, skip_hem, skip_tie):
    def variant(bm):
        ...
        n_steps=16; dip=0.10; thickness=0.018; y_half=0.55
        top_v, bot_v = [], []
        ...
        if not skip_tie:
            for x in support_x[1:-1]:
                bs.add_tie_loop(bm, x, ridge_z-0.004, attach_z-0.002, bs.IRON, seg=8, thick=0.007, bevel_res=2)
    return variant
