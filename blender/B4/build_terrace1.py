"""Batch B4 -- terrace1-3.glb / terrace1-4.glb: 1880s house terrace module.
Ground floor: front door with fanlight + 2 worn steps + railing stub, sash
window beside the door, plinth, string course. Upper floors: two separate
sash windows (rhythm matches the ground-floor door/window below). See
blender/B4/parts.py for the shared geometry helpers.
...
for obj, storeys, wall_top, parapet_top in objs:
    C.export_glb([obj], C.MODELS_DIR + f"/terrace1-{storeys}.glb")
...
for obj, storeys, wall_top, parapet_top in objs:
    ...
    cam_face = C.add_camera("cam_face", C.V(0.0, eye, P.Z_FRONT - 9.0),
                             C.V(0.0, 4.0, P.Z_FRONT), lens=32)
    cam_34 = C.add_camera("cam_34", C.V(3.2, eye, P.Z_FRONT - 9.5),
                           C.V(-0.4, 4.2, P.Z_FRONT + 0.6), lens=32)
    # detail: the door + fanlight + steps together (the defining feature)
    cam_detail = C.add_camera("cam_detail", C.V(-1.55, 1.6, P.Z_FRONT - 3.0),
                               C.V(-1.55, 1.5, P.Z_FRONT), lens=38)
    ...
    for cam, name in ((cam_face, "face"), (cam_34, "34"), (cam_detail, "detail"),
        ...
        C.render_to(C.RENDER_DIR + f"/terrace1-{storeys}_{name}{suffix}.png")
